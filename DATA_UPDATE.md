...
**옛 사본 그대로**다. 새로 넣은 게이트가 아무 말 없이 안 도는 상태가 된다.

**실제로 두 번 났다.**
- 2026-09-02 — 게이트를 추가했는데 사본이 옛것이라 **죽은 수정**이었다. 감시 장치 점검에서 발견.
- 2026-09-08 — 외부 작업자가 훅에 `admissions-links.js` 를 등록했는데 이쪽엔 반영되지 않았다.

**지금은 훅이 스스로 감지한다** — 사본이 저장소본과 다르면 커밋을 막고 위 명령을 알려 준다.
단 **감지하는 쪽은 설치된 사본**이므로, 이 자기 점검이 없던 시절 사본을 쓰고 있으면 감지도 못 한다.
그래서 0단계는 여전히 손으로 한 번 해야 한다.
...
## pre-commit hook (설치 필수 — 위 **0단계** 참조)
...
## 공식 입학처 링크 (admissions-links.js)

`window.IPSI_ADMISSIONS` 는 **확인된 대학만** 공식 입학처·요강 링크를 주고, 나머지는 검색 경로로
떨어뜨린다. 두 가지가 조용히 썩으므로 점검 장치를 둔다.

```bash
python3 qa_official_links.py          # 대학명 키 정합성 + 확인일 신선도(60일)
python3 qa_official_links.py --net    # 위 + 링크 생존(HTTP 200) 확인
```

- ⚠️ **대학명 키는 `data.js` 의 표기와 정확히 같아야 한다.** 어긋나면 그 대학은 공식 링크 대신
  검색 폴백으로 **조용히** 떨어진다(오류도 경고도 없다). 캠퍼스가 나뉜 대학은 별도 항목이다 —
  `고려대학교(세종)`·`연세대학교(미래)`·`한양대학교(ERICA)` 에 본교 링크를 붙이지 마라.
- 링크를 갱신하면 `checkedOn` 도 함께 고친다. 60일이 지나면 하네스가 FAIL 을 낸다.
- 훅에는 넣지 않았다 — 네트워크가 필요해 커밋이 불안정해진다. 링크를 손볼 때 손으로 돌린다.

## 🚨 접수 주간 핫픽스 (동결 중 제보 대응)

**적용 기간: 원서접수 주간(2026-09-07~09-11). 이때는 아래 절차 외의 데이터 변경을 하지 않는다.**
학생이 실제로 원서를 넣는 창이라, 화면의 숫자가 도중에 바뀌면 이미 판단을 끝낸 학생이 흔들린다.

### 0. 고칠 것인가부터 판정한다 (가장 중요)
동결 중 고치는 기준은 "틀렸는가"가 아니라 **"학생을 잘못 인도하는가"** 다.

| 제보 유형 | 조치 |
|---|---|
| 원서접수 기간·마감 **시각** 오류 | **즉시 고친다.** 늦게 안내하면 학생이 접수를 놓친다. |
| 전형 존재/모집단위 누락, 자격 요건 오표기 | **즉시 고친다.** 지원 자체를 그르친다. |
| 수능최저·전형방법 오기 | **즉시 고친다.** |
| 입결 값(등급·환산·경쟁률·추합) 차이 | **미룬다.** 9/12 이후. 판정에 시간이 걸리고, 컷 기준 정의 차이인 경우가 대부분이다. |
| 표현·정렬·UI 개선 | **미룬다.** |

⚠️ 입결 제보를 급히 고치지 마라 — 실측으로 대학이 여러 지표를 공개해 **우리가 옳았던** 사례가
   반복됐다(한밭대 45·세종대 32·목포해양 28·청주대 30건). context-notes (47)(113).

### 1. 근거를 원문에서 확인한다
- **검색 스니펫으로 판정하지 마라.** 세종대 때 스니펫이 어디가 값을 인용해 오판할 뻔했다((113) 전 사례).
- 서열: **대학 공식 최신 요강 > 대학 입학처 공지 > 어디가 > 대행사(진학·유웨이) > 제3자 정리본.**
- 접수 기간·시각은 **접수처(진학/유웨이) 원천값 + 대학 요강** 둘을 본다.

### 2. 최소 변경으로 고친다
- 값 교정은 반드시 `data_corrections.json` 채널로. **`data.js` 를 직접 편집하지 마라**(재빌드에 날아간다).
- `why` 에 날짜·출처 URL·판정 근거를 적는다. 이게 없으면 다음 사람이 같은 건을 또 판다.
- 접수 기간만 바뀐 경우는 `fetch_apply_dates.py` 의 MANUAL 사전에 근거 URL 주석과 함께 넣는다.
...
python3 stamp_assets.py               # 빼먹으면 사용자는 옛 화면을 본다
git add -A && git commit && git push  # 훅이 나머지 하네스를 돌린다
```
★ `--diff` 를 건너뛰지 마라. 동결 중에는 **의도한 행 외에 한 줄도 바뀌면 안 된다.**
   교정 하나가 같은 (대학·학과·전형) 여러 행에 걸리는 일이 있다.

### 4. 배포 후 즉시 육안 확인
브라우저로 실제 배포본을 열고 **해당 대학 카드 하나를 직접 본다.** 캐시버스터가 갱신됐는지도 같이.
`--check` 는 접수 주간 매일 아침 1회:
```bash
python3 fetch_apply_dates.py --check   # 파일을 쓰지 않는다. 종료코드 1 = 마감이 당겨짐 = 위험
```

### 5. 되돌리기
잘못 나갔으면 **논쟁하지 말고 먼저 되돌린다.** 접수 주간엔 원상복구가 최선의 선택지다.
```bash
git revert --no-edit <커밋> && git push
```
`index.html` 의 캐시버스터는 같은 커밋에 들어가므로 **revert 가 함께 되돌린다** —
`stamp_assets.py` 를 다시 돌릴 필요 없다. 되돌린 뒤 `python3 stamp_assets.py --check` 로 확인만 한다.
