...
MANUAL = {
    ...
    # ⚠️ via 정정(2026-09-02): 입학처 공지의 '[수시전형 원서접수 바로가기]' 링크가
    #    https://ipsi2.uwayapply.com/2027/susi1/kaist/?CHA=1 — 유웨이어플라이다. '대학 자체'가 아니었다.
    #    (apply.kaist.ac.kr 은 교사추천서·학교장추천대상 입력 등 부대 절차용이라 접수처와 별개다.)
    # https://admission.kaist.ac.kr/undergraduate/notice/sub03#2269
    'KAIST':    {'from': '2026-09-01T09:00', 'to': '2026-09-09T18:00', 'via': '유웨이어플라이'},
    # 공식 모집요강 p.49 — "2026. 9. 3.(목) 9:00 ~ 9. 10.(목) 18:00까지".
    # ⚠️ 요강은 이미지 PDF(텍스트 층 없음)라 렌더해 판독했다 — p.48 모집일정
    #    '원서접수(온라인/서류제출 포함) 2026. 9. 3.(목) 9:00 ~ 9. 10.(목) 18:00까지'.
    # ⚠️ via 정정(2026-09-02): 같은 요강 p.49 '원서접수 주의사항' 이 "원서접수 시 발생하는 전산상의
    #    문제는 **원서접수 대행업체**에 문의 바람" 이라 자체 접수가 아니다. 유웨이에 2027 수시
    #    디렉터리가 실재한다(https://ipsi3.uwayapply.com/2027/susi1/dgist/).
    # https://www.dgist.ac.kr/thumbnail/mtcltnData/MD_20260429133944989Qt90.pdf
    'DGIST':    {'from': '2026-09-03T09:00', 'to': '2026-09-10T18:00', 'via': '유웨이어플라이'},
    ...
    # ⚠️ via 정정(2026-09-02): 학부입학팀 공지 '2027학년도 학부 수시모집 원서접수 및 서류제출 관련
    #    FAQ'(2026-09-01) 첨부 PDF가 "원서접수 시 **UWAY 사이트**에서 PDF 스캔본 업로드"·"원서접수 시
    #    UWAY 사이트에서 직접 입력" 이라고 세 곳에서 적는다. '대학 자체'가 아니었다.
    # https://adm-u.unist.ac.kr/admission/community/rolling-admissions.do?mode=view&articleNo=306999
    # https://adm-u.unist.ac.kr/admission/guide/admissions-process.do?mode=download&articleNo=105830&attachNo=575153
    'UNIST':    {'from': '2026-09-03T09:00', 'to': '2026-09-10T18:00', 'via': '유웨이어플라이'},
    # 입학팀 공지 '2027학년도 GIST 수시모집 지원 절차 안내'(2026-09-01) —
    # "※ 원서접수 및 서류 제출 기간: 2026.9.7.(월) 09:00 ~ 9.11.(금) 18:00" 로 시작·마감이 한 문장에 있다.
    # 같은 공지에 "▣ 원서접수 및 교사추천서 작성: 유웨이어플라이" 라고 접수처도 명시돼 있어 via 를 정정했다.
    # (요강 p.32 는 마감만 적어 시작 09:00 이 오래 미확인값이었다 — 2026-09-02 해소.)
    # https://www.gist.ac.kr/uadm/html/sub03/0301.html?mode=V&no=223083
    'GIST':     {'from': '2026-09-07T09:00', 'to': '2026-09-11T18:00', 'via': '유웨이어플라이'},
    ...
    # (대시보드 대학 목록에 없어 실제로 쓰이지는 않는다. 요강 표기대로 유웨이로 맞춰 둔다.)
    'KENTECH':  {'from': '2026-09-07T10:00', 'to': '2026-09-11T18:00', 'via': '유웨이어플라이'},
...
def collect():
    ...
    # 시각이 **대학 공표값으로 확인된** 대학 — app.js 의 '시각 미공표' 꼬리표를 끈다.
    # ⚠️ 유래: 꼬리표는 '마감 23:59 = 접수처 기본값' 휴리스틱인데 **반례가 있다.**
    #    신한대 요강 p.18 '2026. 09. 07.(월) 09:00 - 09. 11.(금) 23:59 까지'(문서 내 2곳),
    #    한라대 요강 p.13 '원서접수 기간 : 2026.9.7.(월) 9:00 ~ 9.11.(금) 23:59'.
    #    둘 다 23:59 가 대학이 적은 값이라 꼬리표가 오표시였다(2026-09-04 확인).
    #    ⚠️ 확인된 것만 넣는다 — 경동대·중부대는 아직 근거가 없어 꼬리표를 유지한다.
    for u in ('신한대학교', '한라대학교'):
        if u in out:
