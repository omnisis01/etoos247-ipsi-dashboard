...
# 정상 사유: 인문/자연 분리 모집처럼 요강도 같은 모집단위를 두 줄로 싣는 케이스.
#   · 부산대 자유전공학부 논술: 15+15=30, 요강 논술전형 총계 337+5=342와 대시보드 342 일치 → 정상 확인됨.
#   · 나머지는 (A)3라운드 QA 스캔에서 발견된 미판정분. 요강 전형 총계로 확인 후
#     실중복이면 data_corrections.json의 dedupe로 옮기고 여기서 뺀다.
# 이 목록에 없는 새 중복이 생기면 검증 실패 → 엑셀 갱신 시 회귀를 즉시 잡는다.
DUP_OK = {
    ('부산대학교', '자유전공학부', '논술', '논술전형'),                      # 요강 342 일치, 정상
    ('강원대학교(원주)', '자유전공학부', '학생부교과', '지역교과 I 전형'),   # 이하 미판정
    ('경북대학교', '식품외식산업학과', '학생부교과', '특성화고교졸업자전형(외)'),
    ('단국대학교', '퇴계혁신칼리지(광역)', '논술', '논술우수자전형'),
    ('동명대학교', '군사학과', '학생부종합', '창의인재전형'),
    ('서경대학교', '피아노', '실기/실적', '실기우수자전형'),
    ('안양대학교', '디지털미디어디자인학과', '학생부교과', '아리학생부면접전형'),
    ('안양대학교', '화장품발명디자인학과', '학생부교과', '아리학생부면접전형'),
    ('전주대학교', '인문콘텐츠대학(영어영문,일본언어,\n중국어,역사,웹툰만화,한국어문학)', '학생부교과', '지역인재 I 전형'),
    ('조선대학교', '디자인공학과', '학생부종합', '농어촌학생전형(외)'),
    ('조선대학교', '스포츠산업학과', '학생부종합', '농어촌학생전형(외)'),
    ('조선대학교', '태권도학과', '학생부종합', '농어촌학생전형(외)'),
    ('중부대학교', '항공운항서비스학전공', '학생부교과', '학교생활우수자전형'),
    ('중원대학교', '드론봇군사학과', '학생부교과', '일반전형 II'),
    ('한경대학교', '영미언어문화학과', '학생부종합', '사회통합전형'),
}
